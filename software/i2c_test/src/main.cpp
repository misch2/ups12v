...
uint8_t I2CReadByte(int addr, uint8_t reg) {
...
void I2CWriteByte(int addr, uint8_t reg, uint8_t val) {
  Wire.beginTransmission(addr);
  Wire.write(reg);
  Wire.write(val);
  Wire.endTransmission();
...
uint16_t I2CReadWord(int addr, uint8_t reg) {
  Wire.beginTransmission(addr);
  Wire.write(reg);
  Wire.endTransmission();
...
void DumpBQ25798() {
  ...
  for (uint8_t reg = 0; reg < 0x20; reg++) {
    uint8_t val = I2CReadByte(I2C_ADDR, reg);
  ...
  byte = I2CReadByte(I2C_ADDR, 0x1C);
  ...
  Serial.printf("\tBC1.2_DONE_STAT=%s\n", (byte & 0x01) ? "BC1.2 or non-standard detection complete" : "BC1.2 or non-standard detection NOT complete");

  // FIXME atd?


  word = I2CReadByte(I2C_ADDR, 0x31);
  Serial.printf("REG31_IBUS_ADC: %d mA\n", word);

  word = I2CReadByte(I2C_ADDR, 0x33);
  Serial.printf("REG33_IBAT_ADC: %d mA\n", word);

  word = I2CReadByte(I2C_ADDR, 0x35);
  Serial.printf("REG35_VBUS_ADC: %d mV\n", word);

  // REG37_VAC1_ADC
  word = I2CReadByte(I2C_ADDR, 0x37);
  Serial.printf("REG37_VAC1_ADC: %d mV\n", word);

  // REG39_VAC2_ADC
  word = I2CReadByte(I2C_ADDR, 0x39);
  Serial.printf("REG39_VAC2_ADC: %d mV\n", word);

  // REG3B_VBAT_ADC
  word = I2CReadByte(I2C_ADDR, 0x3B);
  Serial.printf("REG3B_VBAT_ADC: %d mV\n", word);

  // REG3D_VSYS_ADC
  word = I2CReadByte(I2C_ADDR, 0x3D);
  Serial.printf("REG3D_VSYS_ADC: %d mV\n", word);

  // REG3F_TS_ADC
  word = I2CReadByte(I2C_ADDR, 0x3F);
  Serial.printf("REG3F_TS_ADC: %d %\n", word);

  // REG41_TDIE_ADC
  word = I2CReadByte(I2C_ADDR, 0x41);
  Serial.printf("REG41_TDIE_ADC: %d degC\n", word);

  // REG43_D+_ADC
  word = I2CReadByte(I2C_ADDR, 0x43);
  Serial.printf("REG43_D+_ADC: %d mV\n", word);

  // REG45_D-_ADC
  word = I2CReadByte(I2C_ADDR, 0x45);
  Serial.printf("REG45_D-_ADC: %d mV\n", word);

  byte = I2CReadByte(I2C_ADDR, 0x48);
  Serial.printf("REG48_Part_Information: PN=%d [3=BQ25798], DEV_REV=%d [1=BQ25798]\n", (byte & 0x38) >> 3, byte & 0x07);
}

void resetWatchdog() {
  uint8_t byte = I2CReadByte(I2C_ADDR, 0x10);
  byte |= 0x08; // set WD_RST bit
  byte &= ~0x07; // clear WATCHDOG timer
  I2CWriteByte(I2C_ADDR, 0x10, byte);
}
